...
class Moxiecode_ImageManagerPlugin extends Moxiecode_ManagerPlugin {
	...
	function _cropImage($man, $file, $filedata, $result) {
        //var_dump($man, $file, $filedata, $result);
		$ext = getFileExt($file->getName());
		$config = $file->getConfig();
		...
		if ($filedata["temp"]) {
			...
			$tempFile->setTriggerEvents(false);
            //var_dump( $file->getAbsolutePath(), $tempFile->getAbsolutePath(), $filedata["top"], $filedata["left"], $filedata["width"], $filedata["height"], $ext, $config["edit.jpeg_quality"] );
